...
def main(argv):
    ...
    # start = time()
    # dst_homo_blur = cv.blur(img, (31, 31))
    ...
    # start = time()
    # dst_gauss_blur = cv.GaussianBlur(img, (31, 31), 0)
    ...
    # start = time()
    # dst_median_blur = cv.medianBlur(img, 31)
    ...
    dst_bilateral_filter = cv.bilateralFilter(img, 5, 75, 75)
